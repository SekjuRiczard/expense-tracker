...
class PinService {
    ...
    private const CACHE_KEY_PREFIX = 'pin_attempts_user_';
    ...
    private function handleFailedAttempt(User $user): void
    {
        try {
            ...
            if ($attempts >= self::MAX_FAILED_ATTEMPTS) {
                ...
                $this->cache->save(
                    $cacheItem->set($attempts)->expiresAfter(self::LOCKOUT_MINUTES * 60)
                );
            }
        } catch (CacheInvalidArgumentException $exception) {
            throw new RuntimeException('Communication error with the cache during sample clearing.', 0, $exception);
        }
    }

    private function lockOutUser(User $user): void
    {
        $user->setPinLockedUntil((new DateTime())->modify(sprintf('+%d minutes', self::LOCKOUT_MINUTES)));
        $this->entityManager->flush();
    }
