...
The PHP API Docker images install the OPcache extension (`install-php-extensions opcache` in `api/Dockerfile` and `api/Dockerfile.prod`). Local development uses FrankenPHP defaults with `opcache.validate_timestamps=1` so code changes are picked up without restarting the container.

Check OPcache status in the running API container:

```bash
docker compose exec api php -m | grep -i opcache
docker compose exec api php -i | grep -E "opcache.enable|opcache.memory_consumption|opcache.validate_timestamps"
```

Production Compose (`docker-compose.prod.yml`) mounts `api/docker/opcache.prod.ini` with tuned values (`validate_timestamps=0`, 256 MB memory). Do not use production OPcache settings in local dev.

### Frontend performance audits (Lighthouse)

Production preview for Lighthouse runs on port **4173**:

```bash
docker compose up -d frontend_preview
cd frontend
npm run lighthouse
...
After `npm run build`, open `frontend/dist/stats.html` (generated by `rollup-plugin-visualizer`) to inspect bundle composition. Lighthouse reports and `dist/stats.html` are gitignored.
